...
class Application:
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("SIGN-TO-SPEAK: A Vision-Based Real-Time Sign Language to Text and Speech Translation System")
        self.root.protocol('WM_DELETE_WINDOW', self.destructor)
        #self.root.geometry("1300x700")
        # Make full screen
        self.root.attributes('-fullscreen', True)  
        #self.root.state('zoomed')
        # Optional: exit full screen with Escape key
        #self.root.bind("<Escape>", lambda e: self.root.attributes("-fullscreen", False))
        
        # To exit full screen with Escape
        ...
        self.b2 = tk.Button(self.root)
        # MOVED UP from y=700 to y=650
        self.b2.place(x=450, y=650)

        self.b3 = tk.Button(self.root)
        # MOVED UP from y=700 to y=650
        self.b3.place(x=650, y=650)

        self.b4 = tk.Button(self.root)
        # MOVED UP from y=700 to y=650
        self.b4.place(x=850, y=650)

        self.speak = tk.Button(self.root)
        # MOVED IN from x=1305 to x=1180
        self.speak.place(x=1180, y=585)
        self.speak.config(text="Speak", font=("Courier", 20), wraplength=100, command=self.speak_fun)

        self.clear = tk.Button(self.root)
        # MOVED IN from x=1205 to x=1060
        self.clear.place(x=1060, y=585)
        self.clear.config(text="Clear", font=("Courier", 20), wraplength=100, command=self.clear_fun)
        '''
        '''self.T4 = tk.Label(self.root)
